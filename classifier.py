...
def classify_step(instruction_image_path, step_image_path, frame_image_path):
    ...
    prompt = [
        {"role": "user", "content": [
            {"type": "text", "text": "You are an expert origami crafter. Your job is to evaluate when I complete a step of the following instructions:"},
            {"type": "image_url", "image_url": {"url": f"data:image/jpeg;base64,{instruction_image}"}},
            {"type": "text", "text": "This is the current step I am trying to make:"},
            {"type": "image_url", "image_url": {"url": f"data:image/jpeg;base64,{step_image}"}},
            {"type": "text", "text": "This is an image of my progress:"},
            {"type": "image_url", "image_url": {"url": f"data:image/jpeg;base64,{frame_image}"}},
            {"type": "text", "text": "Classify my progress with JUST one of 2 stages: 'In progress' or 'Done'."},
        ]}
    ]
...
def main():
    ...
    for frame in frames:
        ...
        print(f"Frame {frame}: Step {current_step} {result}")
